...
| MARK | [Logo] | Head of Brand Identity | `/api/logo-gen` + `/api/logo-confirm` + `logo-wizard.html` | Package 1 |
| PIXEL | [Brand] | Head of Brand Assets | `brand-kit` | Package 2 |
...
**Package 2:** As Package 1 + testimonials section + service area section. Instruct `one-click-website` to hold image placeholders for PIXEL's assets (AI imagery to be inserted after PIXEL completes).

**Package 3:** As Package 2 + invoke `quote-wizard` to generate and embed the quote form before the footer. Also add "Get a Quote" to navigation.

**Package 4:** As Package 3 + add a "Book Now" placeholder section (`<section id="booking">`) with a Calendly widget placeholder before the footer. BOOK will populate it.
...
- Run `site-audit` to check for broken links, SEO issues, missing meta tags, cookie banner
- Any critical issues found by `site-audit` must be fixed before proceeding to the next agent
...
**Codename:** MARK [Logo]
**Endpoints:** `/api/logo-gen` + `/api/logo-confirm` + `logo-wizard.html`
**Active in:** All packages (runs concurrently with build — non-blocking)

MARK fires immediately after ATLAS deploys. It does NOT block PIXEL or WIRE.

### What MARK does

The client receives a link to their personalised logo wizard in the confirmation email (sent automatically by `/api/paypal-webhook`). For manual Cowork deploys (no PayPal trigger), generate the URL and include it in the handover doc:

```
https://forgeisagentic.tech/logo-wizard.html?slug={client-slug}&name={encoded_business_name}&trade={trade}&color={accent_hex_no_hash}&email={encoded_client_email}&pkg={pkg}
```

**Example:**
```
https://forgeisagentic.tech/logo-wizard.html?slug=daves-plumbing&name=Dave%27s+Plumbing&trade=plumber&color=F59E0B&email=dave%40example.com&pkg=p1
```

The client picks from 3 logo styles rendered live in their brand colours:
- **Style A** — Badge + Wordmark (headers, banners, email footers)
- **Style B** — Circle badge (social profile pictures, favicon)
- **Style C** — Bold wordmark (documents, correspondence)

On confirm: `/api/logo-confirm` stores the choice in Vercel KV and emails Aaron with 9 asset URLs. Aaron re-deploys the site with the chosen logo (2 min step).

**24hr fallback:** If the client doesn't respond, Style A is auto-selected. Site re-deploys automatically.

### MARK during deploy

1. ATLAS deploys with a text placeholder where the logo will go (no blocking)
2. Generate the wizard URL using SCOUT's brief (slug, name, trade, accent colour, email)
3. If PayPal triggered: wizard URL is already in the client confirmation email ✅ — no further action needed
4. If manual deploy: include the wizard URL prominently in the handover document
5. Note in the ATLAS completion signal that MARK is live and the wizard is ready

MARK completion signal (include in ATLAS signal):
```
MARK [Logo]: wizard live — URL in client email / handover doc
24hr auto-select: Style A
```

---

## PIXEL: Head of Brand Assets
...
ATLAS will have already embedded the quote wizard (since `one-click-website` auto-invokes `quote-wizard` for Package 3+). WIRE verifies:
...
Run `site-audit` (a second pass if ATLAS ran one, or first pass for packages that had issues):
- Check all links resolve
- Confirm cookie banner is present and functional
- Confirm Privacy Policy and T&Cs are linked correctly
- Confirm mobile layout renders correctly
- Confirm SEO meta title and description are set
...
MARK: LOGO WIZARD
  Wizard URL: [logo-wizard URL]
  Status: [client notified / awaiting selection / confirmed: Style X]
  Assets: [9 files ready / pending client selection]
...
WHAT TO DO NOW
  1. Send the handover document to the client
  2. Walk them through the website on a call
  3. Set their env vars in Vercel if not already done
  4. Log a 30-day check-in in your calendar
