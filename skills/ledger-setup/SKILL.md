...
- **Supabase table `forge_invoices`** — lives in a dedicated FORGE project (`sfexojjhlzrnldpqamcr`, "forge-agentic-marketing", `eu-west-2`), not shared with any other app. Columns: `client_slug`, `hubspot_deal_id` (unique — this is the idempotency guard), `customer_name`, `customer_email`, `amount_pence`, `status` (outstanding/paid/void), `due_date`, `paid_date`, `invoice_pdf_url`. RLS enabled with zero public policies — only the service role key can touch it, by design. Still on Supabase's free plan, which still auto-pauses on inactivity — dedicating the project removed the "shared with unrelated data" risk but not the pause risk itself. Pro plan is the real fix for that, not yet done.
